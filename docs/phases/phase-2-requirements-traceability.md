...
| 11 | Padding excluded from meaningful load/loss statistics | `losses.py` (unmodified), `moe_diagnostics.py` (valid-token-only aggregates) | `tests/test_model_moe_property.py::test_padding_never_alters_valid_token_statistics_across_many_layouts`, `test_model_fault_injection.py::test_padding_included_in_router_statistics_is_detected` | -- |
| 12 | Full per-token route tracing works | `moe_diagnostics.py::build_token_trace`, `assemble_full_trace` | `tests/test_model_moe_trace.py` | exp-0016 (480/480 records) |
...
| 19 | Expert gradient norms work | -- (standard `p.grad` inspection; no custom accumulator needed) | `tests/test_model_moe.py::test_gradients_reach_router_shared_expert_and_selected_experts_only` (unmodified), `test_model_fault_injection.py::test_missing_expert_gradient_statistics_is_detected` | -- |
| 20 | Routing health detectors work on controlled cases | `moe_diagnostics.py::detect_dead_experts`/`detect_starved_experts`/`detect_dominant_experts`/`detect_routing_collapse`/`detect_router_saturation`/`detect_routing_oscillation` | `tests/test_model_moe_diagnostics.py` (one test per detector, healthy + pathological cases) | exp-0018 |
| 21 | Context-sensitivity probe infrastructure works | `juniper_auto/analysis/context_sensitivity.py` | `tests/test_context_sensitivity.py` (synthetic 3-regime tests, `ProbeCase` validation, model-level harness) | exp-0017; `validate_phase2.py::gate_context_sensitivity_infrastructure` |
...
| 23 | All required ablation modes work | `moe_ablations.py` (6 modes) | `tests/test_model_moe_ablations.py` (one exact-math test per mode) | exp-0019 |
...
| 35 | Required remote CI passes | `.github/workflows/phase-2-validation.yml` | -- | recorded in the metadata-closure commit once GitHub reports the result |
